...
def get_id(model_path):
    ...
    netArc_checkpoint = model_path
    netArc = torch.load(netArc_checkpoint)
    return netArc


def get_tea(model_path):
    save_filename = '%s_net_%s.pth' % ('latest', 'G')
    save_path = model_path + save_filename
    gen = Generator_Adain_Upsample(input_nc=3, output_nc=3, latent_size=512, n_blocks=9, deep=False)
    gen.load_state_dict(torch.load(save_path))
    return gen
